...
5. For Zentao or an equivalent sectioned requirement, read ../../rules/profiles/zentao.md and use the third-part product rules as the default acceptance basis.
6. Read only the other ../../rules/profiles files matched by the requirement.
7. If final cases are requested, hand the structured result to ../qa-testcase-design/SKILL.md.
...
6. For requirement-only input, output the pure-requirement contract: analysis scope, requirement understanding, rule decomposition, evidence, pending questions, risks, test-point summary, and regression scope. Do not require a suspected-defect section.
7. When Diff evidence exists, pass the acceptance baseline to the Diff skill and switch the final report to the combined contract before designing cases.
