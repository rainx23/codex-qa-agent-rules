...
3. Run ../../scripts/validate_xmind_md.py for roots, fixed hierarchy, dimensions, strict three-digit numbering, syntax, duplicate errors/warnings, assertions, unknown-rule leakage, and multi-entry rules. Verify grouped/direct multi-entry paths, at least two peer branches, complete branch step/expected pairs, no mixed direct-step branches, real entrance names, and joined-line detection without rejecting ordinary combination operations.
...
5. Run ../../scripts/md_to_xmind.py and re-read content.json, metadata.json, and manifest.json to compare root, TC count, branch order and total node count. Count TC nodes, not `entry_branches`; compare Testcase Model and XMind dimensions, common-entry/module levels, test point, steps, expected results, and branch count/order/name/content.
