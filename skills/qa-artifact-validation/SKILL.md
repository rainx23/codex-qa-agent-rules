...
7. 仅在 Manifest 校验通过后运行 `../../scripts/build_testcase_index.py`，随后必须运行 `../../scripts/validate_testcase_index.py testcases/index.md`；确认每个 passed Manifest 的 artifact id 和 Manifest 路径均唯一、正式文件存在，且 pending/failed 没有冒充“已校验”。
