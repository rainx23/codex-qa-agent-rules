...
- `extract_candidate`：仅在用户明确要求“提取知识候选”或初始任务明确要求完成后提取时执行。来源只允许 confirmed Fact、resolved Confirmation、confirmed Fact 支撑的 Acceptance Criteria、稳定字段/计算/权限/状态规则及 current Evidence；复用 search/compare 后映射 `create / merge|reject_duplicate / reconfirm / supersede`，只返回 `status=candidate` 的精简清单，不自动 persist，也不修改正式测试产物。
- `validate`：执行知识、Schema、引用、哈希、active 版本、supersedes 环、索引漂移和敏感信息检查。

默认正式任务完成后不执行 search、compare、extract 或 persist。仅当正式产物校验通过、Requirement Model 有明显可复用 confirmed/resolved 规则且无 blocking、skipped core、unresolved core Fact 时，最终回复可只提示一次“本次存在可能可复用的已确认规则，是否提取为知识候选？”。pre_review、无可复用规则、临时/环境/代码行为或用户明确拒绝沉淀时不提示。
