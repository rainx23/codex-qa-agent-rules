...
疑似缺陷必须结构化引用 DEF ID、confirmed Fact ID 和真实 Change ID；只有代码侧证据或低置信度时只能记录风险。
...
`impact_chains`、`risks` 和 `suspected_defects` 必须按 Diff Impact Schema 生成强类型对象；所有 Change、Fact、Risk、Defect 与 Evidence Reference ID 在交接前执行存在性和双向校验。Diff 证据必须定位 Commit、文件、行区间并保存内容哈希，不能只写泛化来源名称。
