...
| `testcase-model.schema.json` | Testcase、条件覆盖、旧单 Scope 与新多 Scope 兼容契约 | 否，由 `scripts/qa_contracts.py` 统一生成 |
...
- `shared_entry_scope` 与 `shared_entry_scopes` 互斥；前者保留历史兼容，后者表达多个独立 6+ 入口范围。
