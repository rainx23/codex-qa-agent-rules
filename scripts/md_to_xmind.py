...
from qa_validation import (
    Node, ValidationError, count_tree_nodes, markdown_tree,
    validate_markdown_file, validate_xmind_archive,
)
...
def write_xmind(root: Node, output: Path) -> None:
    ...
    metadata = {
        "creator": {"name": "Codex QA Markdown to XMind", "version": "2.0.0"},
...
def convert_outline(input_path: Path, outline, output: Path, *, strict: bool = False) -> Path:
    """Convert an already parsed outline without reading the Markdown again."""

    if strict and outline.warnings:
        raise ValidationError(
            f"{input_path}: strict 模式拒绝 {len(outline.warnings)} 个 warning"
        )
    ...
    try:
        validate_xmind_archive(
            output,
            outline.root.title,
            len(outline.tc_nodes),
            count_tree_nodes(outline.root),
...
def convert_file(
    ...
    return convert_outline(input_path, outline, output, strict=strict)
...
def main(argv: list[str] | None = None) -> int:
    ...
    for path in files:
        try:
            ...
            output = (args.output or default_output_path(input_path)).resolve()
            if output.exists() and not args.overwrite:
                raise FileExistsError(f"{output}: 输出已存在；使用 --overwrite 明确覆盖")
            outline = validate_markdown_file(input_path)
            for warning in outline.warnings:
                print(f"WARNING {warning}")
            convert_outline(input_path, outline, output, strict=args.strict)
