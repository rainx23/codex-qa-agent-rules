...
显式“需求预审/评审、完整性检查、缺失冲突歧义检查、只分析问题暂不生成用例”使用 `pre_review`。它复用 Requirement Analysis、Evidence、Fact 和 Confirmation，只输出预审范围、需求理解、已明确内容、问题清单、待确认、风险影响及 `ready / conditionally_ready / not_ready` 结论；不生成 Risk Matrix、Testcase Model、测试用例、XMind、Manifest、Index、SQL 或 API 自动化产物，结束后也不自动进入正式流程。普通“分析需求并编写测试用例”不进入该模式，继续执行下述两阶段链。
...
`qa-knowledge-management` 是按需支撑能力，默认正式任务完成后不自动搜索或提取知识。正式测试产物校验通过、Requirement Model 无 blocking/skipped core/unresolved core Fact，且存在明显可复用的 confirmed Fact 或 resolved Confirmation 时，最终回复只提示一次“本次存在可能可复用的已确认规则，是否提取为知识候选？”，此时不执行 search、compare、extract 或 persist。
