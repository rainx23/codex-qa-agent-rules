...
- **是什么**：6 个可独立触发、可组合执行的 QA Skills，其中 4 个属于核心交付链。
...
其中 `Requirement → Diff → Testcase → Artifact` 是 4 个核心交付链 Skills；知识管理和 API 自动化是按明确意图或证据命中的支撑型 Skills，不是每次任务都执行的主链步骤。
...
| [structured-model-contract.md](rules/core/structured-model-contract.md) | 4 个核心交付链 Skills 的结构化交接模型、生命周期和 Schema 生成规则 |
| [conversation-delivery-contract.md](rules/core/conversation-delivery-contract.md) | 聊天框最终交付、Confirmation 摘要、文件用途和 passed/pending/failed 模板 |
| [api-automation-rules.md](rules/core/api-automation-rules.md) | 接口自动化影响、参数化、固定 Excel 和健康校验规则 |

4 个核心交付链 Skills 通过 Requirement Analysis、Diff Impact、Risk Coverage Matrix、Testcase Model 依次交接。Python 契约定义在 [qa_contracts.py](scripts/qa_contracts.py)，`rules/schemas/*.schema.json` 是生成产物，不是第二份手工维护的规则正文。
...
显式“需求预审/评审、完整性检查、缺失冲突歧义检查”且未要求测试用例/XMind，或明确“只分析问题暂不生成用例”时使用 `pre_review`。它复用 Requirement Analysis、Evidence、Fact 和 Confirmation，只输出预审范围、需求理解、已明确内容、问题清单、待确认、风险影响及 `ready / conditionally_ready / not_ready` 结论；不生成 Risk Matrix、Testcase Model、测试用例、XMind、Manifest、Index、SQL 或 API 自动化产物，结束后也不自动进入正式流程。同时要求“需求评审并生成测试用例/XMind”和普通“分析需求并编写测试用例”均不进入该模式，继续执行下述两阶段链。

用户只需一次授权“按指定规则分析需求并编写测试用例”。系统按“一次授权、两阶段执行、集中确认、确认后自动续跑”处理：

1. 保存完整原始任务范围，读取需求、Diff 和用户补充证据。
2. 第一阶段 `confirmation_only` 完整扫描业务目标、入口、角色、流程、字段、数据、状态、操作、排除项、验收依据和八类测试维度。
3. 建立 confirmed、conflicting、inferred、missing Fact，判断条件矩阵适用性并收集全部 Confirmation。
4. 无 blocking 时自动进入第二阶段；有 blocking 时只保存 Evidence 和最小 Requirement Checkpoint，在一次聊天回复中集中提问，不生成 Risk/Testcase/XMind/Manifest/Index 或对应草稿。
5. 用户可一次回答多个 Confirmation；只更新被回答项，未回答项保持 pending。全部 blocking 解决后自动恢复原始任务，不要求再次授权。
6. 第二阶段生成正式 Requirement 报告、Risk Coverage Matrix、Testcase Model 和固定格式 XMind Markdown。
7. 校验并转换 `.xmind` Workbook，复验完整树、TC 数和节点数。
8. 生成并校验 Manifest，原子更新 `testcases/index.md`。
9. 从 Manifest 和结构化模型确定性渲染最终聊天摘要。

追踪关系必须来自正式 Markdown 表格行，TC 仅出现在普通正文、使用 `TC001-TC010` 范围或缺少风险/证据的文本，都不视为已建立追踪。

```mermaid
flowchart TD
    A["一次授权并保存原始任务范围"] --> B["confirmation_only 完整扫描"]
    B --> C{"存在 blocking Confirmation？"}
    C -- "是" --> D["集中返回全部问题<br/>仅保存 Checkpoint 和 Evidence"]
    C -- "否" --> E["formal_generation 正式阶段"]
    D --> F{"全部 blocking 已解决？"}
    F -- "否" --> D
    F -- "是，自动续跑" --> E
    E --> G["生成正式 Risk 与 Testcase Model"]
    G --> H["生成并校验 XMind 与 Workbook"]
    H --> I["生成 Manifest 并更新索引"]
```

只有阻塞类问题暂停最终用例；非阻塞类和建议确认类可以继续已明确部分。用户跳过的问题仍需保留，假设不能升级为事实。用户回答解除全部 blocking 后，系统必须回写 Confirmation 与关联 Fact、重算 Risk/Testcase，并按原始任务自动继续未完成的正式产物链，不要求用户重复发送“继续生成”。
...
禅道证据优先级不在 README 重复维护；唯一权威顺序及正式来源冲突处理见 [zentao.md](rules/profiles/zentao.md)。
...
全局范围的每个叶子入口都必须独立执行，模拟交易、正式交易等范围必须分别完整展开，禁止使用“上述”“同上”“等入口”、省略号或外部入口清单代替。单入口继续使用原有四层/五层结构，不增加无意义的入口层。入口必须是真实业务名称，不得使用“入口A”“页面1”等占位符；仅当数据来源、权限、条件、预期、异常处理、风险或失败定位不同才拆分为多个 TC。
...
`qa-knowledge-management` 是按需支撑能力，默认正式任务完成后不自动搜索或提取知识。正式测试产物校验通过、Requirement Model 无 blocking/skipped core/unresolved core Fact，且存在明显可复用的 confirmed Fact 或 resolved Confirmation 时，最终回复只提示一次候选数量、必要的简短规则摘要和“是否提取为知识候选？”，此时不执行 search、compare、extract 或 persist。提示仍 pending 时可直接回复“提取/提取这些”；其他上下文中的孤立“提取”不触发知识候选。

只有用户明确要求“提取知识候选”（包括初始任务明确要求完成后提取）才执行 `extract_candidate`。候选只可来自 confirmed Fact、resolved Confirmation、其支撑的验收标准及 current Evidence；不得来自 inferred/missing/conflicting Fact、unresolved/skipped Confirmation、代码推导、临时数据、环境或 pre_review。提取复用现有 search/compare，结果只返回 `status=candidate` 的精简清单，不修改 Requirement/Risk/Testcase/XMind/Manifest；用户明确指定保存候选后才进入现有 persist 流程。
...
- 用户优先使用 `.xmind`、`.xmind.md` 和 `requirement-analysis.md`；聊天摘要会列出完整仓库相对路径、用途和状态。
- Requirement、Risk 和 Testcase JSON 用于结构化追踪和自动校验；Manifest 用于定位全部产物；Index 用于查询历史正式产物及版本关系。
- blocking Confirmation 会直接在聊天框提问；nonblocking 和 suggested 也会在最终摘要中分组展示。没有待确认点时仍明确输出“无”。
- `validation_status` 与 `sql_status` 分开显示，避免把 SQL 未执行误解为测试设计失败。
- 最终摘要由 `scripts/render_delivery_summary.py` 从 Manifest 和模型确定性生成，并由 `scripts/validate_delivery_summary.py` 复验，不依赖自然语言猜测路径、数量或文件用途。
